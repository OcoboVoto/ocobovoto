...
const OPTION_COLORS = ['#6366f1', '#22c55e', '#ef4444', '#f59e0b', '#3b82f6', '#ec4899', '#14b8a6', '#8b5cf6']
  ...
  if (active && payload?.length) {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-3 text-sm">
        ...
        <p className="text-gray-700">{e.value.toFixed(2)}%</p>
        {e.payload.personas !== undefined && <p className="text-gray-500 text-xs">{e.payload.personas} personas</p>}
      </div>
    )
  }
  return null
...
function SvgPieChart({ data, size = 180 }: { data: SvgSlice[]; size?: number }) {
  ...
  const slices = data.map(d => {
    ...
    return { ...d, path: `M ${cx} ${cy} L ${x1.toFixed(2)} ${y1.toFixed(2)} A ${r} ${r} 0 ${large} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z`, lx: cx + r * 0.65 * Math.cos(mid), ly: cy + r * 0.65 * Math.sin(mid), sweep }
  })
    ...
    <svg width={size} height={size} style={{ overflow: 'visible' }}>
      {slices.map((s, i) => (
        <g key={i}>
          ...
          {s.sweep > 0.15 && <text x={s.lx.toFixed(2)} y={s.ly.toFixed(2)} textAnchor="middle" dominantBaseline="central" fill="white" fontSize={11} fontWeight={700}>{s.value.toFixed(1)}%</text>}
        </g>
      ))}
    </svg>
  )
...
function ProposicionScreenChart({ prop }: { prop: any }) {
  const data = [
    ...prop.opciones.map((opc: any, i: number) => ({ name: opc.texto, value: parseFloat(opc.porcentaje.toFixed(2)), personas: opc.personas, color: OPTION_COLORS[i % OPTION_COLORS.length] })),
    { name: 'No votaron', value: parseFloat(prop.noVotaron.porcentaje.toFixed(2)), personas: prop.noVotaron.personas, color: NO_VOTARON_COLOR },
  ]
  ...
  const renderLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, value }: any) => {
    ...
    return <text x={cx + radius * Math.cos(-midAngle * RADIAN)} y={cy + radius * Math.sin(-midAngle * RADIAN)} fill="white" textAnchor="middle" dominantBaseline="central" fontSize={12} fontWeight={600}>{`${value.toFixed(1)}%`}</text>
  }
  return (
    ...
    <ResponsiveContainer width="100%" height={200}>
      <PieChart>
        <Pie data={data} cx="50%" cy="50%" outerRadius={80} dataKey="value" startAngle={90} endAngle={-270} labelLine={false}>
          {data.map((e, i) => <Cell key={i} fill={e.color} />)}
          <text x="50%" y="50%" textAnchor="middle" dominantBaseline="central" fill="#111827" fontSize={18} fontWeight={700}>{pct.toFixed(1)}%</text>
        </Pie>
...
export default function ReportePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const numQuorums = [quorumCierreData, true, quorumFinalData].filter(Boolean).length
        ...
        <div className="bg-white shadow-sm rounded-lg p-8 print:shadow-none print:p-0 print:rounded-none">
          ...
          <section className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><span className="text-gray-600">Fecha:</span><span className="ml-2 font-medium">{format(new Date(asamblea.fechaHora), "d 'de' MMMM 'de' yyyy", { locale: es })}</span></div>
              ...
              {asamblea.registrosCerrados && asamblea.quorumAlCierreRegistros !== null && (
                <div>
                  ...
                  {asamblea.fechaCierreRegistros && (
                    <span className="ml-1 text-xs text-gray-400">({format(new Date(asamblea.fechaCierreRegistros), 'HH:mm', { locale: es })})</span>
                  )}
                </div>
              )}
              ...
              <div>
                ...
                <span className={`ml-2 font-medium ${asamblea.quorumInicial >= asamblea.quorumRequerido ? 'text-green-600' : 'text-red-600'}`}>{asamblea.quorumInicial.toFixed(2)}%</span>
              </div>
              {asamblea.quorumFinal !== null && (
                <>
                  <div>
                    <span className="text-gray-600">Quórum post-confirmación:</span>
                    <span className={`ml-2 font-medium ${asamblea.quorumFinal >= asamblea.quorumRequerido ? 'text-green-600' : 'text-red-600'}`}>{asamblea.quorumFinal.toFixed(2)}%</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Variación (Q2→Q3):</span>
                    <span className={`ml-2 font-medium ${asamblea.quorumFinal < asamblea.quorumInicial ? 'text-red-600' : 'text-green-600'}`}>{(asamblea.quorumFinal - asamblea.quorumInicial).toFixed(2)}%</span>
                  </div>
                </>
              )}
            </div>
            ...
            <div className={`mt-6 grid gap-6 ${numQuorums === 3 ? 'grid-cols-3' : numQuorums === 2 ? 'grid-cols-2' : 'grid-cols-1 max-w-xs'}`}>
              ...
              {quorumCierreData && (
                <>
                  <div className="print:hidden"><p className="text-xs font-semibold text-center text-amber-600 mb-1 uppercase tracking-wide">📋 Quórum al Cierre</p><QuorumScreenChart pct={Number(asamblea.quorumAlCierreRegistros!)} color="#f59e0b" label="Al cierre de registro" /></div>
                  <div className="hidden print:block text-center"><p className="text-xs font-semibold text-amber-600 mb-2 uppercase tracking-wide">Quórum al Cierre</p><div className="flex flex-col items-center gap-3"><SvgPieChart data={quorumCierreData} size={130} /><SvgLegend data={quorumCierreData} /></div></div>
                </>
              )}
              <div className="print:hidden"><p className="text-xs font-semibold text-center text-indigo-600 mb-1 uppercase tracking-wide">{asamblea.registrosCerrados ? 'Quórum Total' : 'Quórum Inicial'}</p><QuorumScreenChart pct={asamblea.quorumInicial} color="#6366f1" label="Quórum total" /></div>
              <div className="hidden print:block text-center"><p className="text-xs font-semibold text-indigo-600 mb-2 uppercase tracking-wide">{asamblea.registrosCerrados ? 'Quórum Total' : 'Quórum Inicial'}</p><div className="flex flex-col items-center gap-3"><SvgPieChart data={quorumInicialData} size={130} /><SvgLegend data={quorumInicialData} /></div></div>
              {asamblea.quorumFinal !== null && quorumFinalData && (
                <>
                  <div className="print:hidden"><p className="text-xs font-semibold text-center text-green-600 mb-1 uppercase tracking-wide">✅ Post-Confirmación</p><QuorumScreenChart pct={asamblea.quorumFinal} color="#22c55e" label="Confirmados presentes" /></div>
                  <div className="hidden print:block text-center"><p className="text-xs font-semibold text-green-600 mb-2 uppercase tracking-wide">Post-Confirmación</p><div className="flex flex-col items-center gap-3"><SvgPieChart data={quorumFinalData} size={130} /><SvgLegend data={quorumFinalData} /></div></div>
                </>
              )}
            </div>
          </section>
          ...
          {/* 2. ASISTENCIA  */}
          <section className="mb-8">
            <h3 className="text-lg font-semibold mb-1 border-b pb-2">2. ASISTENCIA</h3>

            {/* Desglose por tipo (solo si hay poderes) */}
            {resumen.totalAsistentesPorPoder > 0 && (
              <p className="text-xs text-gray-500 mb-4">
                {resumen.totalAsistentesDirecto} asistentes directos · {resumen.totalAsistentesPorPoder} representados por poder
              </p>
            )}
            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                ...
                <div className="space-y-1 max-h-80 overflow-y-auto text-sm print:hidden">
                  {asistentes.map((a, i) => (
                    <div key={i} className={`px-2 py-1.5 rounded-md border transition-colors ${a.tipoAsistencia === 'por_poder' ? 'bg-violet-50 border-violet-100 hover:bg-violet-100' : 'bg-green-50 border-green-100 hover:bg-green-100'}`}>
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-1.5 truncate">
                          <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${a.tipoAsistencia === 'por_poder' ? 'bg-violet-400' : 'bg-green-500'}`} />
                          <span className={`font-medium truncate ${a.tipoAsistencia === 'por_poder' ? 'text-violet-900' : 'text-green-900'}`}>{a.nombreCompleto}</span>
                        </div>
                        <span className={`shrink-0 text-xs font-medium ${a.tipoAsistencia === 'por_poder' ? 'text-violet-500' : 'text-green-600'}`}>{a.torreManzana} {a.aptoCasa}</span>
                      </div>
                      {/* Sub-línea del apoderado */}
                      {a.tipoAsistencia === 'por_poder' && a.apoderado && (
                        <p className="text-xs text-violet-500 mt-0.5 pl-3">
                          Rep. por: <span className="font-medium">{a.apoderado.nombreCompleto}</span>
                        </p>
                      )}
                    </div>
                  ))}
                  ...
                  <tbody>
                    {asistentes.map((a, i) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-green-50'}>
                        <td className="py-1 px-2 border border-green-100">
                          <span className={`font-medium ${a.tipoAsistencia === 'por_poder' ? 'text-violet-900' : 'text-green-900'}`}>{a.nombreCompleto}</span>
                          {a.tipoAsistencia === 'por_poder' && a.apoderado && (
                            <span className="block text-[9px] text-violet-500 mt-0.5">Por poder · {a.apoderado.nombreCompleto}</span>
                          )}
                        </td>
                  ...
                  <tbody>
                    {poderes.map((p, i) => (
                      <tr key={p.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        ...
                        <td className="py-2 px-3 border border-gray-200 text-gray-600 text-xs">{p.otorgante.torreManzana} {p.otorgante.aptoCasa}</td>
                        <td className="py-2 px-3 border border-gray-200 text-right font-medium text-indigo-700">{p.otorgante.coeficiente.toFixed(4)}%</td>
          ...
          <section className="mb-8">
            <h3 className="text-lg font-semibold mb-4 border-b pb-2">{poderes && poderes.length > 0 ? '4' : '3'}. RESULTADOS DE VOTACIONES</h3>
                  ...
                  <div key={prop.id} className="border rounded-lg p-5 print:break-inside-avoid print:border-gray-300">
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1 pr-4">
                        ...
                        <p className="text-xs text-gray-500 mt-1">Mayoría {prop.tipoMayoria} · {prop.porcentajeRequerido}% requerido</p>
                      </div>
                    </div>
                    <div className="print:hidden"><ProposicionScreenChart prop={prop} /></div>
                    <div className="hidden print:grid print:grid-cols-2 print:gap-6 print:items-start">
                      <div className="flex flex-col items-center gap-3"><SvgPieChart data={pieData} size={160} /><SvgLegend data={pieData} /></div>
                        ...
                        <tbody>
                          {prop.opciones.map((opc: any, i: number) => (
                            <tr key={i} className={i % 2 === 0 ? '' : 'bg-gray-50'}>
                              <td className="py-1.5 px-2 border border-gray-200"><div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: OPTION_COLORS[i % OPTION_COLORS.length] }} />{opc.texto}</div></td>
                          ...
                          <tr className="bg-orange-50">
                            <td className="py-1.5 px-2 border border-gray-200"><div className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: NO_VOTARON_COLOR }} />No votaron</div></td>
                      ...
                      {prop.opciones.map((opc: any, i: number) => (
                        <div key={i}>
                          <div className="flex items-center justify-between text-sm mb-1">
                            <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: OPTION_COLORS[i % OPTION_COLORS.length] }} /><span className="font-medium">{opc.texto}</span></div>
                            <div className="text-right"><span className="font-semibold text-gray-900">{opc.porcentaje.toFixed(1)}%</span><span className="text-gray-500 text-xs ml-2">({opc.personas} votos · coef. {opc.coeficiente.toFixed(4)}%)</span></div>
                          </div>
                          <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="h-1.5 rounded-full" style={{ width: `${opc.porcentaje}%`, backgroundColor: OPTION_COLORS[i % OPTION_COLORS.length] }} /></div>
                        </div>
                      ))}
                      <div className="pt-2 border-t">
                        <div className="flex items-center justify-between text-sm mb-1">
                          <div className="flex items-center gap-2"><span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: NO_VOTARON_COLOR }} /><span className="font-medium text-gray-600">No votaron</span></div>
                          <div className="text-right"><span className="font-semibold text-gray-900">{prop.noVotaron.porcentaje.toFixed(1)}%</span><span className="text-gray-500 text-xs ml-2">({prop.noVotaron.personas} personas · coef. {prop.noVotaron.coeficiente.toFixed(4)}%)</span></div>
                        </div>
                        <div className="w-full bg-gray-100 rounded-full h-1.5"><div className="h-1.5 rounded-full" style={{ width: `${prop.noVotaron.porcentaje}%`, backgroundColor: NO_VOTARON_COLOR }} /></div>
                      </div>
                    </div>
                    <p className="text-sm text-gray-700 mt-4 pt-3 border-t"><strong>Opción ganadora:</strong> {prop.opcionGanadora}</p>
                  </div>
                )
              })}
            </div>
          ...
          {/*5. RESUMEN */}
          <section className="mb-8">
            <h3 className="text-lg font-semibold mb-4 border-b pb-2">{poderes && poderes.length > 0 ? '5' : '4'}. RESUMEN</h3>
            <div className="grid grid-cols-4 gap-4 text-center">
              ...
              <div className="bg-green-50 rounded-lg p-4">
                ...
                {resumen.totalAsistentesPorPoder > 0 && (
                  <p className="text-xs text-gray-500 mt-1">{resumen.totalAsistentesDirecto} directos · {resumen.totalAsistentesPorPoder} por poder</p>
                )}
              </div>
          ...
          <div className="mt-12 pt-8 border-t">
            <div className="grid grid-cols-2 gap-8">
              <div className="text-center"><div className="border-t border-gray-400 pt-2 mt-16"><p className="text-sm font-medium">Presidente</p></div></div>
              <div className="text-center"><div className="border-t border-gray-400 pt-2 mt-16"><p className="text-sm font-medium">Secretario</p></div></div>
            </div>
          </div>
