...
function buildPiePaths(data: { value: number; color: string }[], size = 130) {
  ...
  data.forEach((d) => {
    ...
    const pathD = `M ${cx} ${cy} L ${x1.toFixed(2)} ${y1.toFixed(2)} A ${r} ${r} 0 ${large} 1 ${x2.toFixed(2)} ${y2.toFixed(2)} Z`
    paths.push({ d: pathD, fill: d.color })
  })
  return paths
}
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.infoGrid}>
          <Text style={styles.infoItem}>
            ...
            <Text style={{ fontWeight: 'bold', textTransform: 'capitalize' }}>{asamblea.tipo}</Text>
          </Text>
          <Text style={styles.infoItem}>
          ...
          <Text style={styles.infoItem}>
            ...
            <Text style={{ fontWeight: 'bold', textTransform: 'capitalize' }}>{asamblea.modalidad}</Text>
          </Text>
          <Text style={styles.infoItem}>
            <Text style={styles.infoLabel}>Estado: </Text>
            <Text style={{ fontWeight: 'bold', textTransform: 'capitalize' }}>{asamblea.estado}</Text>
          </Text>
          <Text style={styles.infoItem}>
            <Text style={styles.infoLabel}>Quórum requerido: </Text>
            <Text style={{ fontWeight: 'bold' }}>{asamblea.quorumRequerido}%</Text>
          </Text>
          {asamblea.registrosCerrados && asamblea.quorumAlCierreRegistros !== null && (
            <Text style={styles.infoItem}>
              ...
              <Text
                style={{
                  fontWeight: 'bold',
                  color: Number(asamblea.quorumAlCierreRegistros) >= asamblea.quorumRequerido ? '#16a34a' : '#dc2626',
                }}
              >
          ...
          <Text style={styles.infoItem}>
            <Text style={styles.infoLabel}>
              {asamblea.registrosCerrados ? 'Quórum total acumulado:' : 'Quórum inicial:'}{' '}
            </Text>
            <Text
              style={{
                fontWeight: 'bold',
                color: asamblea.quorumInicial >= asamblea.quorumRequerido ? '#16a34a' : '#dc2626',
              }}
            >
            ...
            <>
              <Text style={styles.infoItem}>
                ...
                <Text
                  style={{
                    fontWeight: 'bold',
                    color: asamblea.quorumFinal >= asamblea.quorumRequerido ? '#16a34a' : '#dc2626',
                  }}
                >
              ...
              <Text style={styles.infoItem}>
                ...
                <Text
                  style={{
                    fontWeight: 'bold',
                    color: asamblea.quorumFinal < asamblea.quorumInicial ? '#dc2626' : '#16a34a',
                  }}
                >
        ...
        <View style={styles.asistenciaGrid}>
          <View style={styles.asistenciaCol}>
            <Text style={[styles.colTitle, { color: '#15803d' }]}>Asistentes ({asistentes.length})</Text>
            ...
            {asistentes.map((a, i) => (
              <View key={i} style={[styles.tableRow, { backgroundColor: i % 2 === 0 ? '#fff' : '#f0fdf4' }]}>
                <Text style={[styles.tableCell, { flex: 2, fontWeight: 'bold' }]}>{a.nombreCompleto}</Text>
          ...
          <View style={styles.asistenciaCol}>
            <Text style={[styles.colTitle, { color: '#dc2626' }]}>No Asistentes ({noAsistentes.length})</Text>
            ...
            <View style={styles.poderesTable}>
              ...
              {poderes.map((p, i) => (
                <View key={i} style={[styles.poderesRow, { backgroundColor: i % 2 === 0 ? '#fff' : '#f9fafb' }]}>
                  ...
                  <Text style={[styles.poderesCell, { width: 60, textAlign: 'right', fontWeight: 'bold', color: '#4f46e5' }]}>
        ...
        {/* 4. Resultados votaciones */}
        <Text style={[styles.sectionTitle, { marginTop: poderes.length > 0 ? 0 : 24 }]}>
          {secNum(4)}. RESULTADOS DE VOTACIONES
        </Text>
        {proposiciones.map((prop) => {
          ...
          return (
            <View key={prop.id} style={styles.proposicionBox} wrap={false}>
              <Text style={styles.proposicionTitle}>
                {prop.numeroOrden}. {prop.titulo}
              </Text>
              {prop.descripcion && (
                <Text style={{ fontSize: 10, color: '#4b5563', marginBottom: 4 }}>{prop.descripcion}</Text>
              )}
              <Text style={styles.proposicionMeta}>
                Mayoría {prop.tipoMayoria} · {prop.porcentajeRequerido}% requerido
              </Text>
              <View style={styles.proposicionContent}>
                <View style={styles.proposicionPie}>
                ...
                <View style={styles.proposicionTable}>
                  ...
                  <View style={[styles.poderesRow, { backgroundColor: '#fff7ed' }]}>
                    ...
                    <Text style={[styles.poderesCell, { width: 55, textAlign: 'right' }]}>
                      {prop.noVotaron.coeficiente.toFixed(4)}%
                    </Text>
                  </View>
                </View>
              </View>
              <Text style={styles.opcionGanadora}>
                <Text style={{ fontWeight: 'bold' }}>Opción ganadora: </Text>
                {prop.opcionGanadora}
              </Text>
            </View>
          )
        ...
        <View style={styles.firmas}>
          <View style={styles.firmaBox}>
            <View style={styles.firmaLine}>
              <Text style={styles.firmaText}>Presidente</Text>
            </View>
          </View>
          <View style={styles.firmaBox}>
            <View style={styles.firmaLine}>
              <Text style={styles.firmaText}>Secretario</Text>
            </View>
          </View>
        </View>
...
export async function generarReporteHTML(asambleaId: string): Promise<Buffer> {
  ...
  const pdfBuffer = await renderToBuffer(<ReportePDF reporte={reporte} />)
  return pdfBuffer
}
